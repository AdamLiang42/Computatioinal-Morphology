...
class Encoder_gru(nn.Module):
    def __init__(self, vocab, vocab_size, embedding_size, hidden_size, dropout_p=0):
        ...
        self.lstm = nn.GRU(embedding_size, hidden_size, batch_first=True, bidirectional=True)
...
class Decoder_gru(nn.Module):
    def __init__(self, vocab, embedding_size, hidden_size, use_hierarchical_attention, use_ptr_gen, max_decode_len=25, epsilon=1e-6, dropout_p=0):
        ...
        if use_hierarchical_attention:
            self.lstm = nn.GRU(embedding_size + 2*hidden_size, hidden_size, batch_first=True)
            ...
            self.lstm = nn.GRU(embedding_size + 4*hidden_size, hidden_size, batch_first=True)
            if use_ptr_gen:
    ...
    def forward(self, lemma_input_indices, h_l, h_tg, mask_l, mask_tg, decoder_initial, inputs=None, a_ls_true=None, p_gens_true=None):
        ...
        for t in range(max_t):
            ...
            _, s = self.lstm(torch.cat([x, h_c_star], dim=1).unsqueeze(1), s+c)
...
class Model(nn.Module):
    def __init__(self, vocab, embedding_size, hidden_size, use_hierarchical_attention, use_ptr_gen, dropout_p=0):
        ...
        self.vocab = vocab
        self.lemma_encoder = Encoder_gru(vocab, vocab.char_vocab_size, embedding_size, hidden_size, dropout_p=dropout_p)
        self.tag_encoder = Encoder_gru(vocab, vocab.tag_vocab_size, embedding_size, hidden_size, dropout_p=dropout_p)
        self.bridge_h = nn.Linear(2*hidden_size, hidden_size)
        self.bridge_c = nn.Linear(2*hidden_size, hidden_size)
        self.decoder = Decoder_lstm(vocab, embedding_size, hidden_size, use_hierarchical_attention, use_ptr_gen, dropout_p=dropout_p)
