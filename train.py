...
def train_and_evaluate(input_train, labels_train, input_val, labels_val, model, optimizer, loss_fn, epochs=1, batch_size=32, clip=None, show_progress=True, writer=None, model_save_dir=None, starting_epoch=1, initial_best_val_acc=-0.01):
    ...
    # record max accuracy for early stop
    max_accuracy = 0
    min_loss = 0
    not_update_count = 0
    ...
    for epoch_num in t:
        ...
        if metrics["val acc"] > max_accuracy:
            max_accuracy = metrics["val acc"]
            not_update_count = 0
        elif metrics["val loss"] < min_loss:
            min_loss = metrics["val loss"]
            not_update_count = 0
        else:
            not_update_count += 1
        if not_update_count > 10 and max_accuracy > 0:
            break
        '''
...
def train_model(language, dataset, lr, epochs, embedding_size, hidden_size, clip, dropout_p, alpha, beta, show_progress = True):
    ...
    model = package.net.Model(vocab, embedding_size=embedding_size, hidden_size=hidden_size, use_ptr_gen=False, dropout_p=dropout_p, use_hierarchical_attention=True).to(device)
...
def get_languages(dataset):
    ...
    for filename in os.listdir(TASK1_DATA_PATH):
        if 'train-'+dataset in filename:
            languages.append(filename.split('-train-' + dataset)[0])
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        ...
        # calculate average accuracy and distance
        print("="*100)
        print("Data size: {}, Accuracy: {}, Distance: {}".format(dataset, sum(accuracy_collection)/len(accuracy_collection), sum(distance_collection)/len(distance_collection)))